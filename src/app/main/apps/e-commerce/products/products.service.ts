...
export class EcommerceProductsService implements Resolve<any> {
    ...
    resolve(
        ...
        return new Promise((resolve, reject) => {
            Promise.all([this.getEventsByUser()]).then(() => {
                resolve();
            }, reject);
        });
    }
    ...
    getAllEvents(): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            this._httpClient
                .get(environment.apiUrl + "/api/all-events")
                .subscribe((response: any) => {
                    let eventArray: any = [];
                    let count = 0;
    ...
    deleteEvent(product) {
        return new Promise((resolve, reject) => {
            this._httpClient
                .delete(environment.apiUrl + "/api/delete-event/" + product._id)
                .subscribe((response: any) => {
                    resolve(response);

                    this.deleteAllInputsByEvent(product).then((res) => {
                        this.deleteAllInvitedByEvent(product).then((data) => {
