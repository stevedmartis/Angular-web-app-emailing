...
export class CampaignService {
    ...
    getDataInvitedForSendEmail(option, campaign, allInvited) {
        const totalCount = allInvited.length;
        ...
        if (option === "all") {
            ...
            this.sendManyArray(allInvited, campaign, totalCount);
            ...
            arrayInvitedSelected.forEach((obj) => {
                const filter = allInvited.filter((element) => element._id === obj);

                arrayNewSelection.push(filter[0]);
            });

            this.sendManyArray(arrayNewSelection, campaign, totalCount);
...
sendManyArray(allInvited, campaign, totalCount){

    const arraydataImport = allInvited.map((obj) => {
        let final = {
            data: obj.dataImport,
            id: obj._id,
    ...
    arraydataImport.forEach((element) => {
        Object.getOwnPropertyNames(element.data[0]).forEach((val) => {
        ...
        this.sendManiInvited(this.eventObj.handle, campaign2, inv)
            ...
            .then((mail) => {
                ...
                this._contactService
                    .editMessageId(inv.idInvited, id)
                    .then(() => {
    ...
    removeDuplicateObjects(array: any[]) {
        return [
            ...new Set(array.map((s) => JSON.stringify(s.emailInvited))),
        ].map((s) => JSON.parse(s));
    }
                ...
                if (obj.emailInvited) {
                    this.sendInvited(invitation, obj)
                        .then((mail) => {
                            ...
                            this._contactService
                                .editMessageId(obj.idInvited, id)
                                .then(() => {
                                    this._productService.Is_mailJet();
                                });
    ...
    sendManiInvited(eventName, campaign, invited): Promise<any> {
        return new Promise((resolve, reject) => {
            this._httpClient
                .post<any>(environment.apiUrl + "/api/send-invited", {
                    eventName,
                    campaign,
                    invited,
                })
